...
print(
    "The shape of our training set: %s professionals and %s features"
...
def main():
    ...
    Taken_inputs_from_seniors_or_elders = st.selectbox(
        "Took advice from seniors or elders", ("Yes", "No")
    )
    ...
    smart_or_hard_work = st.selectbox(
        "Smart or Hard Work", ("Smart worker", "Hard Worker")
    )
    ...
    st.write("You selected: **{}**".format(Management_or_Technical))
    ...
    if st.button("Predict"):
        ...
        sns.heatmap(corr, square=True, annot=True, linewidth=0.4, center=2, ax=axes)
